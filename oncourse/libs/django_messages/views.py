...
from oncourse.libs.django_messages.forms import ComposeForm
from oncourse.libs.django_messages.utils import format_quote, get_user_model, get_username_field
...
def inbox(request, template_name='django_messages/inbox.html'):
    ...
    """
    message_list = Message.objects.filter(recipient=request.user.username,
            recipient_deleted_at__in=(None,""))
    return render(request, template_name, {
        'message_list': message_list,
        'process':'message_inbox'
    })
...
def outbox(request, template_name='django_messages/outbox.html'):
    ...
    Optional arguments:
        ``template_name``: name of the template to use.
    """
    message_list = Message.objects.filter(sender=request.user.username,
            sender_deleted_at__in=(None,""))
    return render(request, template_name, {
        'message_list': message_list,
        'process':'message_outbox'
    })
...
def reply(request, message_id, form_class=ComposeForm,
    ...
    if parent.sender != request.user.username and parent.recipient != request.user.username:
        raise Http404
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            form.save(sender=request.user.username, parent_msg=parent)
...
def view(request, message_id, form_class=ComposeForm, quote_helper=format_quote,
    ...
    now = timezone.now()
    message = Message.objects.get(id=message_id)
    if (message.sender != user.username) and (message.recipient != user.username):
        raise Http404
    
    if message.read_at is None and message.recipient == user.username:
        message.read_at = now
        message.save()

    context = {'message': message, 'reply_form': None, 'process':'message_inbox'}
