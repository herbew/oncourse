...
from oncourse.libs.modeltranslation import settings
from oncourse.libs.modeltranslation.fields import TranslationField
from oncourse.libs.modeltranslation.utils import (build_localized_fieldname, get_language,
...
def get_translatable_fields_for_model(model):
    from oncourse.libs.modeltranslation.translator import NotRegistered, translator
    try:
...
class MultilingualQuerySet(models.query.QuerySet):
    ...
    def __reduce__(self):
        return multilingual_query_set_factory, (self.__class__.__bases__[0],), self.__getstate__()
    ...
    def values(self, *fields):
        if not self._rewrite:
            return super(MultilingualQuerySet, self).values(*fields)
        if not fields:
            # Emulate original query_set behaviour: get all fields that are not translation fields
...
def multilingual_query_set_factory(old_cls, instantiate=True):
...
class MultilingualQuerysetManager(models.Manager):
    ...
    def get_queryset(self):
        ...
        return self._patch_query_set(qs)

    def _patch_query_set(self, qs):
        qs.__class__ = multilingual_query_set_factory(qs.__class__, instantiate=False)
        qs._post_init()
...
class MultilingualManager(MultilingualQuerysetManager):
    ...
    def get_queryset(self):
        """
        This method is repeated because some managers that don't use super() or alter query_set class
        may return query_set that is not subclass of MultilingualQuerySet.
        """
