...
def _get_avatars(user):
    ...
    if settings.AVATAR_MAX_AVATARS_PER_USER == 1:
        ...
        # the query_set for the primary avatar
            ...
            for id in ids:
                try:
                    ...
                    if obj.avatar:
                        ...
                        dirs = name.replace("/%s" % file_name, "")
                        
                        #delete resized/130
                        name_130 = os.path.join(dirs,"resized",'130', file_name)
                        name_120 = os.path.join(dirs,"resized",'120', file_name)
                        name_80 = os.path.join(dirs,"resized",'80', file_name)
                            
                        try:
                            cloud_storage = default_storage
                            
                            if cloud_storage.exists(name_130):
                                cloud_storage.delete(name_130)
                            if cloud_storage.exists(name_120):
                                cloud_storage.delete(name_120)
                            if cloud_storage.exists(name_80):
                                cloud_storage.delete(name_80)
                        ...
                        try:
                            if obj.avatar.storage.exists(name_130):
                                obj.avatar.storage.delete(name_130)
                        
                            if obj.avatar.storage.exists(name_120):
                                obj.avatar.storage.delete(name_120)
                                
                            if obj.avatar.storage.exists(name_80):
                                obj.avatar.storage.delete(name_80)
                            
                            # SOME TIme google any miss about charge
                            if obj.avatar.storage.exists(name):
                                obj.avatar.storage.delete(name)
                        except:
                            pass
                    obj.delete()
...
def render_primary(request, user=None, size=settings.AVATAR_DEFAULT_SIZE):
    ...
    if avatar:
        ...
        default_url = settings.DEFAULT_MALE_URL if user.gender == 'male' else settings.DEFAULT_FEMALE_URL
        url = get_default_avatar_url(default_url=default_url)
