...
from oncourse.libs.avatar.conf import settings
from oncourse.libs.avatar.util import (get_primary_avatar, get_default_avatar_url,
                         cache_result, get_user_model, get_user, force_bytes)
from oncourse.libs.avatar.models import Avatar
...
def avatar_url(user, size=settings.AVATAR_DEFAULT_SIZE):
    ...
    try:
        default_url = settings.DEFAULT_MALE_URL if user.gender == 'm' else settings.DEFAULT_FEMALE_URL
    except:
        default_url = settings.DEFAULT_MALE_URL
       
    return get_default_avatar_url(default_url=default_url)
